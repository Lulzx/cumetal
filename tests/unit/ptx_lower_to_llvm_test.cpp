...
)
{
    ...
    if (!expect(contains(lowered.llvm_ir, "; ptx.lower opcode=air.read.special_register"),
                "lowered instruction comments emitted")) {
        return 1;
    }
    if (!expect(!lowered.warnings.empty(), "warnings propagated from pass pipeline")) {
        return 1;
    }
    ...
    const auto strict = cumetal::ptx::lower_ptx_to_llvm_ir(ptx, strict_options);
