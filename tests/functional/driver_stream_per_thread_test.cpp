...
int main(int argc, char** argv) {
    ...
    for (std::size_t i = 0; i < kElementCount; ++i) {
        host_a[i] = static_cast<float>((i * 5) % 19) * 0.5f;
        host_b[i] = static_cast<float>((i * 9) % 23) * 1.25f;
    }
    ...
    if (cuLaunchKernel(vector_add,
        ...
        std::fprintf(stderr, "FAIL: cuLaunchKernel(CU_STREAM_PER_THREAD) failed\n");
        return 1;
    }

    const CUresult query = cuStreamQuery(CU_STREAM_PER_THREAD);
    if (query != CUDA_SUCCESS && query != CUDA_ERROR_NOT_READY) {
        std::fprintf(stderr, "FAIL: cuStreamQuery(CU_STREAM_PER_THREAD) returned %d\n", query);
        return 1;
    }
    ...
    std::printf("PASS: driver CU_STREAM_PER_THREAD launch/query/synchronize\n");
    return 0;
}
