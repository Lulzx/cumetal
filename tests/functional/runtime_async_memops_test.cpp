...
int main(int argc, char** argv) {
    ...
    for (std::size_t i = 0; i < kElementCount; ++i) {
        host_a[i] = static_cast<float>((i * 11) % 37) * 0.5f;
    ...
    std::vector<std::uint8_t> memset_probe(kElementCount * sizeof(float));
    if (cudaMemset(dev_c, 0xAB, memset_probe.size()) != cudaSuccess) {
        std::fprintf(stderr, "FAIL: cudaMemset failed\n");
        return 1;
    }

    if (cudaMemcpy(memset_probe.data(), dev_c, memset_probe.size(), cudaMemcpyDeviceToHost) !=
        cudaSuccess) {
        std::fprintf(stderr, "FAIL: cudaMemcpy after cudaMemset failed\n");
        return 1;
    }

    for (std::size_t i = 0; i < memset_probe.size(); ++i) {
        if (memset_probe[i] != 0xAB) {
            std::fprintf(stderr, "FAIL: cudaMemset pattern mismatch at byte %zu\n", i);
            return 1;
        }
    }
    ...
    if (cudaMemsetAsync(dev_c, 0x00, kElementCount * sizeof(float), stream) != cudaSuccess) {
        std::fprintf(stderr, "FAIL: cudaMemsetAsync failed\n");
        return 1;
    }

    if (cudaMemcpyAsync(dev_a,
                        host_a.data(),
                        kElementCount * sizeof(float),
                        cudaMemcpyHostToDevice,
                        stream) != cudaSuccess ||
        cudaMemcpyAsync(dev_b,
                        host_b.data(),
                        kElementCount * sizeof(float),
                        cudaMemcpyHostToDevice,
                        stream) != cudaSuccess) {
        std::fprintf(stderr, "FAIL: cudaMemcpyAsync host->device failed\n");
        return 1;
    }
    ...
    if (cudaMemcpyAsync(host_c.data(),
                        dev_c,
                        kElementCount * sizeof(float),
                        cudaMemcpyDeviceToHost,
                        stream) != cudaSuccess) {
        std::fprintf(stderr, "FAIL: cudaMemcpyAsync device->host failed\n");
        return 1;
    }
    ...
    std::printf("PASS: async memcpy/memset flow produced correct output\n");
    return 0;
}
