...
int main(int argc, char** argv) {
    ...
    for (std::size_t i = 0; i < kElementCount; ++i) {
        host_a[i] = static_cast<float>((i * 17) % 23) * 0.75f;
        host_b[i] = static_cast<float>((i * 7) % 19) * 1.5f;
    }
    ...
    CUdeviceptr packed_args[] = {dev_a, dev_b, dev_c};
    ...
    if (cuLaunchKernel(vector_add,
                       ...
                       nullptr,
                       extra) != CUDA_SUCCESS) {
        std::fprintf(stderr, "FAIL: cuLaunchKernel(extra packed args) failed\n");
        return 1;
    }
    ...
    std::printf("PASS: cuLaunchKernel supports extra packed argument path\n");
    return 0;
}
