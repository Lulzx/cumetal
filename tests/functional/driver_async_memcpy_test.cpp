...
int main(int argc, char** argv) {
    ...
    for (std::size_t i = 0; i < kElementCount; ++i) {
        host_a[i] = static_cast<float>((i * 11) % 41) * 0.5f;
    ...
    if (cuMemcpyHtoDAsync(dev_a, host_a.data(), kElementCount * sizeof(float), stream) != CUDA_SUCCESS ||
        cuMemcpyHtoDAsync(dev_b, host_b.data(), kElementCount * sizeof(float), stream) != CUDA_SUCCESS) {
        std::fprintf(stderr, "FAIL: cuMemcpyHtoDAsync failed\n");
        return 1;
    }
    ...
    if (cuMemcpyDtoDAsync(dev_tmp, dev_c, kElementCount * sizeof(float), stream) != CUDA_SUCCESS) {
        std::fprintf(stderr, "FAIL: cuMemcpyDtoDAsync failed\n");
        return 1;
    }

    if (cuMemcpyDtoHAsync(host_out.data(), dev_tmp, kElementCount * sizeof(float), stream) !=
        CUDA_SUCCESS) {
        std::fprintf(stderr, "FAIL: cuMemcpyDtoHAsync failed\n");
        return 1;
    }
    ...
    if (cuMemFree(dev_a) != CUDA_SUCCESS || cuMemFree(dev_b) != CUDA_SUCCESS ||
        cuMemFree(dev_c) != CUDA_SUCCESS || cuMemFree(dev_tmp) != CUDA_SUCCESS) {
    ...
    std::printf("PASS: driver async memcpy APIs preserve stream ordering\n");
    return 0;
}
