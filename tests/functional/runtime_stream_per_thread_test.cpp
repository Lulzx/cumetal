...
int main(int argc, char** argv) {
    ...
    for (std::size_t i = 0; i < kElementCount; ++i) {
        host_a[i] = static_cast<float>((i * 3) % 17) * 1.5f;
        host_b[i] = static_cast<float>((i * 7) % 19) * 0.25f;
    }
    ...
    if (cudaLaunchKernel(&kernel, grid_dim, block_dim, launch_args, 0, cudaStreamPerThread) !=
        cudaSuccess) {
        std::fprintf(stderr, "FAIL: cudaLaunchKernel(cudaStreamPerThread) failed\n");
        return 1;
    }

    const cudaError_t query_status = cudaStreamQuery(cudaStreamPerThread);
    if (query_status != cudaSuccess && query_status != cudaErrorNotReady) {
        std::fprintf(stderr, "FAIL: cudaStreamQuery(cudaStreamPerThread) returned %d\n", query_status);
        return 1;
    }
    ...
    std::printf("PASS: runtime cudaStreamPerThread launch/query/synchronize\n");
    return 0;
}
