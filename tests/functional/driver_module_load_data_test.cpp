...
int main(int argc, char** argv) {
    ...
    std::ifstream in(metallib_path, std::ios::binary);
    std::vector<std::uint8_t> data((std::istreambuf_iterator<char>(in)),
                                   std::istreambuf_iterator<char>());
    if (data.empty()) {
        std::fprintf(stderr, "FAIL: failed to read metallib bytes\n");
        return 1;
    }

    CUmodule module = nullptr;
    if (cuModuleLoadData(&module, data.data()) != CUDA_SUCCESS || module == nullptr) {
        std::fprintf(stderr, "FAIL: cuModuleLoadData failed\n");
        return 1;
    }
    ...
    if (cuModuleUnload(module) != CUDA_SUCCESS) {
        std::fprintf(stderr, "FAIL: cuModuleUnload after loadData failed\n");
        return 1;
    }

    if (cuModuleLoadDataEx(&module, data.data(), 0, nullptr, nullptr) != CUDA_SUCCESS ||
        module == nullptr) {
        std::fprintf(stderr, "FAIL: cuModuleLoadDataEx failed\n");
        return 1;
    }
    ...
    if (cuModuleUnload(module) != CUDA_SUCCESS) {
        std::fprintf(stderr, "FAIL: cuModuleUnload after loadDataEx failed\n");
        return 1;
    }
